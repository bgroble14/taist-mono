...
<body>
    ...
    <div class="wrapper">        
        ...
        <form class="slogin div_login" method="POST" action="/forgot" id="reset_form">
            {{ csrf_field() }}
            <?php if (isset($_GET['success'])) { ?>
                <div class="font_medium fsize22 mb24 tcenter">We've sent a link to reset password to your email</div>
                <div class="flex flex_jcenter">
                    <a class="bt" href="/login">SIGN IN</a>
                </div>
            <?php } else { ?>
                <div class="font_medium fsize22 mb24">FORGOT PASSWORD</div>
                <div class="d_input mb16">
                    <input class="form-control f_input" id="email" name="email" placeholder="Enter email" />
                </div>
                <div class="error mb4">
                    @if($errors->any())
                ...
                <div class="bt" id="bt_forgot">SEND</div>
            <?php } ?>
        </form>
    </div>
