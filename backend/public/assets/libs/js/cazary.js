...
(function($, window, undefined)
{
	...
	var checkEmail = (function()
	{
		...
		return function(string)
		{
			...
			{
				// length is up to 256 charactersï¼ˆcf. RFC 5321 4.5.3.1.3.ï¼‰
				return false;
			}
			if(string.indexOf("@") > 64)
			{
				// length of local part is up to 64 charactersï¼ˆcf. RFC 5321 4.5.3.1.1.ï¼‰
				return false;
			}
	...
	var EditorCore = (function()
	{
		...
		var
			...
			COMMAND_INSERTATTACH         = "insertattach",
		...
		return function(edit, value, style)
		{
			...
			this.COMMAND_INSERTATTACH          = COMMAND_INSERTATTACH;
...
//				contentDocument.body.contentEditable = true;
			...
			function _execCommand(commandName, parameters)
			{
				...
				_setFocus();
				if (commandName == "insertattach") {
					fname = parameters.split('/');
					fname = fname[fname.length - 1];
					contentDocument.execCommand("insertHTML", false, `
						<div class='attachment' contenteditable="false" style="border: 1px solid #aaa;padding:5px 10px; background: #e0e0e0;display: inline-block;margin: 10px 0px;">
								<div style="display: flex;align-items: center; margin-bottom: 3px;">
									<img src='images/attach.png' style="width: 20px;"/> Attachment
								</div>
							<a href="`+parameters+`" target="_blank" download>
								<div style="font-size: 14px;font-style: italic;color: #555;">`+fname+`</div>
							</a>
						</div>
					`);
				} else {
...
*/
			...
			var PRE_DEFINED_MACROS = {
				...
				"FULL": [
					...
					"inserthorizontalrule insertimage insertattach createlink unlink",
					"undo redo",
					"source"
				]
			};
			...
			return function(options)
			{
				...
				return this.each(function()
				{
					...
					var commands_with_panel = [
						...
						editor.COMMAND_INSERTIMAGE, editor.COMMAND_CREATELINK, editor.COMMAND_INSERTATTACH
					];
					...
					function createPanel(commandName, options, $command)
					{
						...
						case editor.COMMAND_INSERTATTACH:
							$panel = createPanel_insertattach(commandName);
							break;
...
*/
					...
					function createPanel_insertattach(commandName)
					{
						$panel = $("<div>")
							.append(
								$("<form />")
										...
										$("<div />")
											.append(
												$("<fieldset />")
													.append(
														$("<legend />")
															.text(_("Input Attachment URL"))
													)
													.append(
														$("<input type=\"text\" style='min-width: 300px;' />")
															.addClass("cazary-widget-insertattach-url")
															.attr({
																"required": "required",
																"placeholder": _("http://example.com/path/to/attachment.pdf")
															})
													)
											)
											.append(
						...
						function onsubmit()
						{
							var $url = $panel.find(".cazary-widget-insertattach-url");
							var  url = $url.val();
							if(!checkURL(url))
							{
								$url.trigger("focus");
